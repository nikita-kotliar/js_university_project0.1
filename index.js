import{r as w,f as S,a as v,b as L,h as E,p as T}from"./assets/header-D5NR5ieG.js";import{i as u}from"./assets/vendor-iVKk4foX.js";const r={filters:document.querySelector(".filters"),musclesBtn:document.querySelector(".muscles-btn"),exercisesTitle:document.querySelector(".exercises-title"),searchForm:document.querySelector(".search-form"),quoteContainer:document.querySelector(".quote"),exercises:document.querySelector(".exercises-cards"),pagination:document.querySelector("#pagination-container")};let n=1,q=window.innerWidth<768?9:12,C=window.innerWidth<768?8:10,f="Muscles",l="",o="",p="",h=[];w(r.quoteContainer);r.musclesBtn.classList.add("active-btn");m();r.filters.addEventListener("click",F);r.exercises.addEventListener("click",$);r.searchForm.addEventListener("input",k);r.pagination.addEventListener("click",H);async function m(t=!0){t&&(n=1,r.exercises.innerHTML="");const e=await S(f,n,q,f);if(!e.results.length)return y();M(e.results),b(e.totalPages)}function M(t){r.exercises.innerHTML=`
    <ul class="exercises">
  ...
  `,r.pagination.innerHTML=""}function z(t){return t[0].toUpperCase()+t.slice(1)}const a=document.querySelector("input[name=email]"),c=document.querySelector(".footer-send-button"),x="feedback-form-state",I=()=>{localStorage.setItem(x,JSON.stringify({email:a.value}))},W=()=>{const t=localStorage.getItem(x);if(!t)return;const{email:e}=JSON.parse(t);a.value=e||"",c.disabled=!a.value};W();a.addEventListener("input",()=>{I(),c.disabled=!a.value});c.addEventListener("click",async t=>{if(t.preventDefault(),!!a.value)try{await T(a.value),u.success({title:"Success",message:"Welcome to energy.flow world!",position:"topRight"}),a.value="",c.disabled=!0,localStorage.removeItem(x)}catch(e){e.message==="EMAIL_EXISTS"?u.warning({title:"Warning",message:"Email already exists",position:"topRight"}):u.error({title:"Error",message:"Something went wrong! Please try again later",position:"topRight"})}});
