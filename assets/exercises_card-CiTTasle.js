...
`,F=async e=>{const{quote:t,author:s}=await M();e.innerHTML=O(t,s)},p={root:document.querySelector(".scrollbar_custom"),quoteContainer:document.querySelector(".quote")};let a=1;const h=8,T=(e,t)=>{const s=(t-1)*h;return e.slice(s,s+h)},B=(e=[])=>{const t=new Set;return e.filter(s=>!(s!=null&&s._id)||t.has(s._id)?!1:(t.add(s._id),!0))};function C(){p.root.innerHTML=`
  ...
  `;let o=Math.max(1,a-1),i=Math.min(e,a+1);a===1&&(i=Math.min(e,3)),a===e&&(o=Math.max(1,e-2)),o>1&&(t+=s(1),o>2&&(t+='<li class="dots">...</li>'));for(let r=o;r<=i;r++)t+=s(r);return i<e&&(i<e-1&&(t+='<li class="dots">...</li>'),t+=s(e)),t+="</ul>",t}function g(){if(!p.root)return;const e=B(y(m)||[]);if(!e.length){C();return}const t=Math.ceil(e.length/h);a>t&&(a=t);const s=window.innerWidth<1440?T(e,a):e;p.root.innerHTML=A(s)+(window.innerWidth<1440?P(t):"")}var S;(S=p.root)==null||S.addEventListener("click",e=>{const t=e.target.closest('[data-action="delete"]'),s=e.target.closest('[data-action="start"]'),o=e.target.closest(".pagination-btn");if(t&&(L(t.dataset.id),g()),s){const r=(y(m)||[]).find(n=>n._id===s.dataset.id);r&&K(r,!0,!0)}if(o){const i=Number(o.dataset.page);i!==a&&(a=i,g())}});window.addEventListener("resize",()=>{a=1,g()});g();F(p.quoteContainer);async function H(e,{email:t,rate:s,comment:o}){const i=`https://your-energy.b.goit.study/api/exercises/${e}/rating`;s=Number(s);const r=await fetch(i,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,rate:s,review:o})});if(!r.ok){const n=await r.json();throw new Error(n.message||"Rating failed")}return r.json()}const J=document.getElementById("form-close-btn"),u=document.querySelector(".backdrop"),j=document.querySelector("#user-email"),q=document.getElementById("user-comment"),Q=document.querySelector(".form-send-btn"),R=document.querySelector(".rating-wrapper"),E=document.querySelector(".rating-star-value"),D=document.querySelector(".backdrop-form");let $=null;const c={rate:0,email:"",comment:""};Q.disabled=!1;function U(){j.value="",q.value="",c.rate=0,c.comment="",c.email="",E.textContent="0.0",document.querySelectorAll(".rating-star-icons").forEach(t=>{t.style.fill="var(--white-20)"})}J.addEventListener("click",()=>{u.classList.remove("is-open")});u.addEventListener("click",e=>{e.target===u&&u.classList.remove("is-open")});R.addEventListener("click",e=>{const t=document.querySelectorAll(".rating-star-icons");if(e.target.dataset.id){c.rate=Number(e.target.dataset.id);for(let s=0;s<5;s++)t[s].style.fill=s<c.rate?"var(--star-color)":"var(--white-20)";E.textContent=`${c.rate}.0`}});function W(e){$=e,u.classList.add("is-open")}D.addEventListener("submit",Y);async function Y(e){if(e.preventDefault(),c.email=j.value.trim(),c.comment=q.value.trim()||void 0,!c.rate){alert("Please select a rating");return}if(!c.email){alert("Please enter your email");return}try{await H($,c),alert("Your rating is accepted"),U(),u.classList.remove("is-open")}catch(t){alert(`Error: ${t.message}`)}}const l=document.querySelector(".exr-card-backdrop");let d=!1,f=[],_=JSON.parse(localStorage.getItem("favourite"));_&&_.forEach(e=>{f[0]||(f[0]=e),f.push(e)});function v(e){return`${e.charAt(0).toUpperCase()}${e.slice(1)}`}function K(e,t=!1,s=!1){d=t,d||f.forEach(o=>{o._id===e._id&&(d=!0)}),z(e),l.classList.add("card-is-open"),document.body.classList.add("not-scrollable"),d===!0&&(document.querySelector(".add-favourite-btn").innerHTML=`Remove from
    ...
    </div>`;l.innerHTML=o;const i=document.querySelectorAll(".star-rating-icon");for(let n=0;n<Math.round(e.rating);++n)i[n].style.fill="#eea10c";const r=document.querySelector(".add-favourite-btn");r.addEventListener("click",function(){d?(L(e._id),r.innerHTML=`Add to favourite
          <svg class="heart-icon" width="20px" height="20px">
            <use href="/js_university_project0.1/symbol-defs.svg#icon-heart"></use>
          </svg>`,d=!1):(f.push(e),w(f),r.innerHTML=`Remove from
          <svg class="heart-icon" width="20px" height="20px">
            <use href="/js_university_project0.1/symbol-defs.svg#icon-heart"></use>
          </svg>`,d=!0),g()}),document.getElementById("close-card").addEventListener("click",()=>{l.classList.remove("card-is-open"),document.body.classList.remove("not-scrollable")}),l.addEventListener("click",n=>{n.target===l&&(l.classList.remove("card-is-open"),document.body.classList.remove("not-scrollable"))}),document.querySelector(".give-rating-btn").addEventListener("click",()=>{l.classList.remove("card-is-open"),document.body.classList.remove("not-scrollable"),W(e._id)})}export{F as d,K as h};
//# sourceMappingURL=exercises_card-CiTTasle.js.map
